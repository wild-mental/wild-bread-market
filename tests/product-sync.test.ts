...
function fakeMall(overrides: Partial<Cafe24Product> = {}) {
  ...
  return {
    ...
    // 다음 count번 조회가 snapshot(기본값: 지금 값)을 돌려주게 한다.
    staleNextReads: (count: number, snapshot?: Cafe24Product) => {
      stale = { snapshot: { ...(snapshot ?? product) }, remaining: count };
    },
  };
}
...
describe('수정 직후 조회가 늦게 반영될 때(카페24 실측 현상)', () => {
  ...
  it('적용 뒤 복원인데 조회가 이전 값(기준값)을 주면, 다시 조회해 실제값으로 복원한다', async () => {
    const mall = fakeMall();
    await saveBaseline(mall.deps);
    const beforeApply = { ...mall.product };
    mall.putResponse('full'); // 적용은 PUT 응답으로 확인 → 조회가 늦어도 applied가 저장된다
    await applyChange(mall.deps, 'down', true);
    mall.staleNextReads(2, beforeApply); // 복원 직전 조회 2번은 아직 5,200원

    const restored = await restoreBaseline(mall.deps);
    expect(restored.result).toBe('applied');
    expect(restored.before.price).toBe(4940); // "이미 기준값"으로 속지 않는다
    expect(mall.sleeps.slice(0, 2)).toEqual([1000, 2000]);
    expect(mall.product.price).toBe('5200.00');
    expect(mall.state()?.applied).toBeNull();
  });

  it('정말 기준값으로 돌아와 있으면 다시 조회한 뒤 already로 끝낸다', async () => {
    const mall = fakeMall();
    await saveBaseline(mall.deps);
    mall.putResponse('full');
    await applyChange(mall.deps, 'down', true);
    mall.product.price = '5200.00'; // 사람이 관리자 화면에서 기준값으로 되돌림
    mall.product.summary_description = 'Sample Product Generated by CAFE24';

    const restored = await restoreBaseline(mall.deps);
    expect(restored.result).toBe('already');
    expect(mall.sleeps).toEqual([1000, 2000, 3000]); // 3번 더 조회해도 기준값
    expect(mall.state()?.applied).toBeNull();
  });

  it('적용 기록이 지워졌어도 앱이 만든 값이면 복원한다(충돌로 막지 않는다)', async () => {
    const mall = fakeMall();
    await saveBaseline(mall.deps);
    await applyChange(mall.deps, 'down', true);
    const state = mall.state()!;
    await mall.deps.saveState({ ...state, applied: null, scenario: null }); // 잘못된 already로 지워진 상태
  ...
  it('사람이 직접 바꾼 값이면 적용 기록과 무관하게 복원을 멈춘다(409)', async () => {
    const mall = fakeMall();
    await saveBaseline(mall.deps);
    await applyChange(mall.deps, 'down', true);
    mall.product.price = '4500.00'; // 앱이 만들 수 없는 값
    const state = mall.state()!;
    await mall.deps.saveState({ ...state, applied: null, scenario: null });
    await expect(restoreBaseline(mall.deps)).rejects.toMatchObject({ status: 409, code: 'EDITED_ELSEWHERE' });
    expect(mall.product.price).toBe('4500.00');
  });

  it('적용한 적 없는데 "이미 목표값"으로 읽히면 다시 조회하고, 실제로 다르면 수정한다', async () => {
    const mall = fakeMall();
    await saveBaseline(mall.deps);
    await applyChange(mall.deps, 'down', true);
    const applied = { ...mall.product };
    await restoreBaseline(mall.deps); // applied 기록 없음 · 몰은 5,200원
    mall.staleNextReads(1, applied); // 직전 적용값(4,940)이 아직 읽힘
    const result = await applyChange(mall.deps, 'down', true);
    expect(result.result).toBe('applied'); // already로 끝내지 않는다
    expect(mall.sleeps.slice(0, 1)).toEqual([1000]);
  });

  it('같은 적용을 반복하면(적용 기록이 목표값과 같으면) 다시 조회하지 않고 already', async () => {
    const mall = fakeMall();
    await saveBaseline(mall.deps);
    mall.putResponse('full');
    await applyChange(mall.deps, 'down', true);
    const sleepsBefore = mall.sleeps.length;
    const again = await applyChange(mall.deps, 'down', true);
    expect(again.result).toBe('already');
    expect(mall.sleeps.length).toBe(sleepsBefore);
    expect(mall.puts).toHaveLength(1);
  });
