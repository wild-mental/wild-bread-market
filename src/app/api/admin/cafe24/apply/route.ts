...
import { applyChange } from '@/lib/cafe24/product-sync';

// 목표값은 브라우저가 보낸 숫자가 아니라 서버가 기준값에서 다시 계산한다.
export async function POST(request: Request) {
  ...
  try {
    const result = await applyChange(productDeps(guard.userId), body.scenario, body.applyDiscount === true);
