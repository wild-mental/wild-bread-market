...
- POST-05 (2026-09-16) · 프로토타입에 Supabase 준비 자동화(`npm run db:setup` · `/admin/setup`)를 추가하면서 가이드에는 **B 경로 안내만** 넣었다: 5쪽 B 설명, 6쪽 비밀값 표에 `SUPABASE_DB_URL`(내 컴퓨터 전용), 4-2·4-3·5-3 안내 상자, 검증 범위(검증한 방법·실제 Supabase 미접속). 1~3장 🤖 프롬프트와 계약(lab-context)은 바꾸지 않았으므로 A 경로 학습자 프로젝트에는 이 기능이 없고 4장 수작업을 따른다. 프롬프트 텍스트 바이트 동일, GUIDE_CHECK slides=63 errors=0 overflow=0 prompts=24 fullcode=0 appOrigin=OK.
