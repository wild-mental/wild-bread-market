...
| 구분 | 상태 |
|---|---|
| 로컬 검증 | 끝남 — `npm test` 89개 통과, `npm run build`·`npm run lint` exit 0, `node scripts/smoke.mjs` 28/28 PASS |
...
- 같은 이유로 [적용]·[복원] 직후 [상품 조회]나 두 번째 [적용]은 **1~2분 뒤** 누른다. 바로 누르면 이전 값이 보여 `already`·`EDITED_ELSEWHERE` 판단이 틀릴 수 있다.
...
| `EDITED_ELSEWHERE` 409 | 마지막 적용 뒤 누군가 직접 수정, 또는 직전 수정이 조회에 아직 반영 안 됨 | 1~2분 뒤 [상품 조회]로 값 확인 → 사람이 바꾼 값이면 쇼핑몰 관리자에서 정리 후 복원 |
