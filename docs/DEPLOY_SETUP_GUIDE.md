...
| 구분 | 상태 |
|---|---|
| 로컬 검증 | 끝남 — `npm test` 95개 통과, `npm run build`·`npm run lint` exit 0, `node scripts/smoke.mjs` 28/28 PASS |
...
- 적용·복원 결과의 `verifiedBy`는 반영을 무엇으로 확인했는지다. `put_response`는 카페24 수정 응답, `reread`는 다시 조회(`readAttempts`번째에 확인)다. 카페24 조회 API는 수정 직후 1~2분 동안 이전 값과 새 값을 번갈아 돌려줄 수 있어서, 앱이 수정 응답을 먼저 보고 없으면 약 10초 동안 최대 5번 다시 조회한다.
- [적용]·[복원]을 연달아 눌러도 된다. 저장된 적용 기록과 조회값이 어긋나면 앱이 1·2·3초 간격으로 다시 조회해 확인하므로 응답이 몇 초 늦을 수 있다. 다만 [상품 조회] 화면의 현재값은 카페24 조회 그대로라서 수정 직후 1~2분은 이전 값이 보일 수 있다.
...
| `EDITED_ELSEWHERE` 409 | 현재값이 이 앱이 만들 수 있는 값(시나리오 × 할인)이 아님 — 사람이 관리자 화면에서 바꾼 값 | 쇼핑몰 관리자에서 값을 확인하고 기준값(5,200원·원래 요약설명)으로 정리한 뒤 복원 |
| `WRITE_FAILED` 502 · `detail.errorCode` `VERIFY_MISMATCH` | 수정 요청은 성공했지만 약 10초 동안 다시 조회해도 이전 값 | 1~2분 뒤 [상품 조회]. 목표값이면 반영된 것이므로 쇼핑몰 관리자에서 값 확인 후 필요하면 복원 |
| `WRITE_FAILED` 502 · `detail.errorCode` `HTTP_4xx:…` | 카페24가 수정 요청을 거절 | `CAFE24_4xx` 행과 같은 원인 확인 |
| `WRITE_UNKNOWN` 500 | 다시 조회한 값이 이전 값도 목표값도 아니거나 조회 실패 | [상품 조회]로 현재값 확인, 쇼핑몰 관리자에서 값 정리 |
