...
- [ ] TODO-00 `[확인]` 이 저장소를 내 계정으로 가져와 로컬 검증을 통과시킨다: GitHub에서 `wild-mental/wild-bread-market`을 fork(또는 clone 후 내 비공개 저장소로 push) → `npm ci` → `npm test && npm run build && npm run lint && node scripts/smoke.mjs` → 마지막 줄 `SMOKE: 24/24 PASS` (Google Chrome 필요)
...
- [ ] TODO-05 `[웹]` SQL Editor에서 `supabase/migrations/0001_cafe24_lab.sql` 전체를 실행한다 (4-2)
- [ ] TODO-06 `[확인]` 확인 쿼리 결과가 `cafe24_change_log · cafe24_connections · cafe24_products` 3개와 `anon_can_read = false`이다 (4-2)
- [ ] TODO-07 `[웹]` Authentication > Users > Add user에서 관리자 계정(이메일·강한 비밀번호)을 **Auto Confirm User** 체크로 만들고, 비밀번호는 비밀번호 관리자에 저장한다. 권장: 새 사용자 가입(Allow new users to sign up)을 끈다 (4-3)
...
- [ ] TODO-14 `[웹]` 배포 주소 `/login`으로 로그인 → `/admin/cafe24`에 표시된 UUID를 `ADMIN_USER_IDS`에, Production 주소를 `APP_BASE_URL`(끝에 `/` 없이)에 넣고 재배포한다 (5-3)
...
- [ ] TODO-18 `[비밀값]` [STEP 1. 개발정보관리]의 Client ID·Client Secret을 확인해 **Client Secret은 비밀번호 관리자**에 저장하고, 두 값을 Vercel(Production)에 넣고 2차 배포한다 (6-4)
...
- [ ] TODO-24 `[웹]` 실습이 끝나면 위젯 삭제·상품 복원·테스트 앱 삭제·토큰 삭제로 정리하고, 내 컴퓨터의 `.env.local`을 지운다(연동을 계속 운영하면 위젯·앱·토큰은 남긴다) (8-5, 10-3)
...
- [ ] TODO-28 `[비밀값]` 비밀값을 교체할 때: 새 값 발급 → 비밀번호 관리자 갱신 → Vercel 값 교체 → Redeploy → 관리 화면 [연결 상태]로 동작 확인 → 이전 값 폐기. 토큰 암호화 키를 바꾼 경우에만 `cafe24_connections` 행 삭제 후 [카페24 연결하기]를 다시 한다 (비밀값 관리 한눈에)
